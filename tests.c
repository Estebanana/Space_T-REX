...
/**
 *  \brief fonction test qui initialise et affiche un sprite donné en paramètre avec les fonctions init_sprite et print_sprite
*/
...
/**
 *  \brief fonction test qui initialise et affiche deux sprites donné en paramètre et vérifie leur collision
*/
 ...
 *  \brief fonction test qui initialise et affiche deux sprites donné en paramètre et vérifie leur collision
*/
void test_handle_sprites_collision_param(world_t *world, sprite_t *sp1, sprite_t *sp2, int x, int y, int w, int h, int x2, int y2, int w2, int h2){
    init_sprite(sp1, x, y, w, h);
    init_sprite(sp2, x2, y2, w2, h2);
    handle_sprites_collision(world, sp1,sp2);
    printf("vy nulle si collision : %d -- make_disappear : %d\n", world->vy, world->make_disappear);
}
...
void test_handle_sprites_collision(){
    ...
    world->spaceship = malloc(sizeof(sprite_t));
    world->mur = malloc(sizeof(sprite_t));
    test_handle_sprites_collision_param(world, world->spaceship, world->mur, 50,50,10,10,50,50,10,10);
    test_handle_sprites_collision_param(world, world->spaceship, world->mur, 50,50,10,20,150,50,20,10);
    //clean_data(world);
}
